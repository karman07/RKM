...
${billEl.outerHTML}
  ...
  const itemRows = items.map((item) => {
    ...
    return {
      ...
      finalSelling,
      netWeight:   (item.net_weight   || product?.net_weight)   ?? 0,
      grossWeight: (item.gross_weight || product?.gross_weight) ?? 0,
      stoneWeight: (item.stone_weight || product?.stone_weight) ?? 0,
