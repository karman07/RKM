...
import {
  ...
  AlignLeft, AlignCenter, AlignRight, Bold, Layers,
} from 'lucide-react';
...
const BARCODE_TYPE_META: Record<BarcodeType, { label: string; bcid: string; square?: boolean }> = {
  code128: { label: 'Barcode — Code128', bcid: 'code128' },
  code39:  { label: 'Barcode — Code39', bcid: 'code39' },
  ean13:   { label: 'Barcode — EAN-13', bcid: 'ean13' },
  qrcode:  { label: 'QR Code', bcid: 'qrcode', square: true },
};
...
function barcodeUrl(text: string, type: BarcodeType) {
  const meta = BARCODE_TYPE_META[type];
  const base = `https://bwipjs-api.metafloor.com/?bcid=${meta.bcid}&text=${encodeURIComponent(text)}`;
  return meta.square ? `${base}&scale=3` : `${base}&scale=5&height=15&includetext`;
}
...
function defaultElements(w: number, h: number): DesignElement[] {
  ...
  const fields: DesignElement[] = fieldKeys.map((kind, i) => ({
    id: kind, kind, visible: true,
    x: fx, y: 0.5 + i * rowH, w: fw, h: rowH,
    bold: true, align: 'left', fontMm: null, text: '', barcodeType: 'code128',
  }));
...
export default function LabelDesignerModal({ open, mode, singleItem, bulkItems, onClose }: LabelDesignerModalProps) {
  ...
  function addCustomText() {
    const newEl: DesignElement = {
      ...
      bold: false, align: 'left', fontMm: null, text: 'Custom text', barcodeType: 'code128',
    };
      ...
      return (
        ...
        >
          <img
            ...
            src={barcodeUrl(item.barcode, el.barcodeType)}
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="w-full flex gap-3">
              <button
                ...
                disabled={!canPrint}
                className="flex-1 py-4 rounded-2xl bg-blue-600 text-white text-[11px] font-bold uppercase tracking-widest shadow-xl hover:bg-blue-700 transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:active:scale-100"
              >
