      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={addSplit}
            className="flex items-center gap-1 px-2.5 py-1 bg-[#5A0F1A] hover:bg-[#7A1C2A] text-white text-[10px] font-black rounded-lg transition-all"
          >
      ...
      <div className="space-y-2">
        {splits.map((split, idx) => (
          <div key={idx} className="flex gap-2 items-center bg-slate-50 border border-slate-100 rounded-xl p-2.5">
            <div className="flex-shrink-0 w-6 h-6 rounded-lg bg-[#5A0F1A]/10 flex items-center justify-center text-[10px] font-black text-[#5A0F1A]">
              {idx + 1}
            </div>

            <select
              className="flex-shrink-0 px-2 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#5A0F1A]/40 min-w-[110px]"
              value={split.mode}
            ...
            <div className="relative flex-1 min-w-0">
              ...
              <input
                ...
                min="0"
                step="1"
                className="w-full pl-6 pr-2 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-black text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#5A0F1A]/40"
                placeholder="Amount"
                value={split.amount}
            ...
            <input
              type="text"
              className="flex-1 min-w-0 px-2 py-1.5 rounded-lg border border-slate-200 bg-white text-xs text-slate-600 focus:outline-none focus:ring-2 focus:ring-[#5A0F1A]/40 placeholder-slate-300"
