...
import { searchCustomerByPhone, type FullCustomer } from '@/lib/api';
...
export default function VerifiedCustomerPanel({ value, onChange, onVerifiedChange }: Props) {
  ...
  function getOrCreateRecaptcha() {
    if (!(window as any)._rcv_invoice_customer) {
      (window as any)._rcv_invoice_customer = new RecaptchaVerifier(auth, 'recaptcha-invoice-customer', { size: 'invisible' });
    }
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      const result = await signInWithPhoneNumber(auth, `+91${newPhone.replace(/^\+91/, '')}`, verifier);
