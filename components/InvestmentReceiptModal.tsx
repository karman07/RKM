...
import type { GoldSubscription } from '@/lib/api';

interface InvestmentReceiptModalProps {
  sub: GoldSubscription;
  balance: number;
  onClose: () => void;
}
    ...
    <div className="fixed inset-0 z-[200] flex flex-col items-center bg-black/70 p-2 md:p-4 overflow-y-auto">
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1.5px solid #000' }}>
          <div style={{ padding: '12px 20px', borderRight: '1px solid #ccc' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: '12px', marginBottom: '2px' }}>{sub.customerName}</div>
            {sub.customerPhone && <div>Phone: {sub.customerPhone}</div>}
            {sub.customerEmail && <div>Email: {sub.customerEmail}</div>}
          </div>
