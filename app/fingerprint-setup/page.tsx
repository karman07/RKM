  ...
  async function handleRegister() {
    ...
    try {
      ...
      const regRes = await fetch(`${API_BASE}/auth/webauthn/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${setupData.setup_token}` },
      ...
      const loginRes = await fetch(`${API_BASE}/auth/login`, {
        ...
        body: JSON.stringify({ email: setupData.email, password: setupData.password, latitude: setupData.coords?.latitude, longitude: setupData.coords?.longitude }),
      });
      ...
      if (loginBody.webauthn_required) {
        ...
        const authRes = await fetch(`${API_BASE}/auth/webauthn/authenticate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${loginBody.pending_token}` },
        ...
        localStorage.setItem('manager_session', JSON.stringify({
          ...
          role: authBody.user.role, name: authBody.user.name,
          id: authBody.user._id || authBody.user.id, branch_id: authBody.user.branch_id,
        }));
      ...
      setErrorMsg(err?.name === 'NotAllowedError'
        ? 'Fingerprint scan was cancelled. Please tap the button and scan your fingerprint when prompted.'
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-8 space-y-6">
          <div className="flex justify-center">
            <div className="w-28 h-28 rounded-full bg-[#5A0F1A]/5 border-2 border-[#5A0F1A]/20 flex items-center justify-center relative">
              ...
              {status === 'registering' && <div className="absolute inset-0 rounded-full border-4 border-[#5A0F1A]/20 border-t-[#5A0F1A] animate-spin" />}
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-1.5">
            ...
            {['Tap the button below', 'Your device will prompt for fingerprint / Face ID', 'Scan your finger on the sensor', "You're all set — always use this fingerprint to log in"].map((step, i) => (
              <div key={i} className="flex items-start gap-2 text-[11px] text-slate-600">
                <span className="font-black text-[#5A0F1A] flex-shrink-0">{i + 1}.</span>{step}
              </div>
            ))}
          </div>
          ...
          {status === 'error' && (
            <div className="bg-red-50 border border-red-100 rounded-2xl px-4 py-3 text-xs font-bold text-red-700 flex items-start gap-2">
              <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className="flex-shrink-0 mt-px"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /></svg>
              {errorMsg}
            </div>
          )}
          ...
          {status === 'done' ? (
            <div className="flex items-center justify-center gap-3 py-2">
              <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className="text-emerald-600"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
              </div>
              <p className="text-sm font-black text-emerald-700">Fingerprint registered! Signing you in...</p>
            </div>
          ) : (
            <>
              <button onClick={handleRegister} disabled={status === 'registering' || !setupData}
                ...
                {status === 'registering' ? (
                  ...
                  <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 10a2 2 0 0 0-2 2v.5" /><path d="M8 10a4 4 0 0 1 8 0v4.5" /></svg>Scan My Fingerprint</>
                )}
              </button>
              {status === 'error' && (
                <button onClick={() => setStatus('idle')} className="w-full h-9 rounded-xl border border-slate-200 text-xs font-bold text-slate-500 hover:bg-slate-50 transition-colors">Try Again</button>
              )}
            </>
          )}
