  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          if (data.branchLat != null) {
            // Rich geofence error — show coord comparison
            setGeoError({
              message: msg,
      ...
      if (user?.role !== 'manager') {
        throw new Error('Access Denied: This portal is for Manager accounts only. Admins must use the Admin Panel.');
      }
      ...
      <PageShell>
        <div className="text-center space-y-6">
          ...
          <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">
            Sign-in is restricted to branch locations
          </p>
        </div>
      </PageShell>
    );
        ...
        <div className="text-center space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 text-left space-y-2">
            ...
            {loc.status === 'denied' ? (
              ...
              <ol className="space-y-1.5 text-[11px] font-medium text-amber-800">
                <li className="flex items-start gap-2"><span className="font-black text-amber-600 flex-shrink-0">1.</span>Ensure your device's GPS / location is turned on</li>
    ...
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm font-medium text-slate-500">Your session refreshes daily at 9:00 AM</p>
