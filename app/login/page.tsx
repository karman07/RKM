...
function FingerprintStep({
  ...
  async function handleScan() {
    setScanning(true);
    try {
      // Trigger platform biometric (Touch ID / Face ID / Windows Hello / Android fingerprint)
      ...
      if (err?.name === 'NotAllowedError') {
        onError('Fingerprint scan was cancelled or timed out. Please try again.');
      } else {
        onError(err?.message || 'Biometric verification failed.');
      }
    } finally {
      setScanning(false);
    }
  ...
  return (
    <div className="space-y-6 text-center">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1 leading-relaxed">Use your fingerprint to complete sign-in</p>
      </div>
      <button
        onClick={handleScan}
        disabled={scanning}
...
export default function CashierLogin() {
  ...
  const [freshCoords, setFreshCoords] = useState<{ latitude: number; longitude: number }>({ latitude: 0, longitude: 0 });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (body.needs_webauthn_setup) {
        // First login — go to fingerprint registration page
      ...
      if (body.webauthn_required) {
        // Fingerprint already registered — scan inline
      ...
      // Admin / custom role — direct token
    ...
    return (
      <PageShell>
        <div className="text-center space-y-6">
          ...
          <button onClick={askLocation} className="w-full h-[52px] bg-[#5A0F1A] hover:bg-[#7A1C2A] text-white rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-[#5A0F1A]/25 flex items-center justify-center gap-2">
      ...
      <PageShell>
        {authError ? (
          <div className="space-y-4">
            ...
            <button
              onClick={() => { setWebauthnStep(null); setAuthError(''); }}
              className="w-full h-10 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors"
            >
              Back to Login
            </button>
          </div>
          ...
          <FingerprintStep
            ...
            coords={freshCoords}
            onSuccess={data => completeLogin(data, freshCoords)}
              ...
              <div className="bg-red-50 border border-red-200 rounded-2xl overflow-hidden">
                ...
                <div className="mx-4 mb-4 space-y-2 bg-white border border-red-100 rounded-xl p-3">
                  {geoError.userLat != null && (
                    <CoordRow icon={<LocationIcon className="w-3 h-3 text-blue-600" />} label="Your Location" lat={geoError.userLat} lng={geoError.userLng!} color="bg-blue-50" />
                  )}
                  {geoError.branchLat != null && (
                    <CoordRow icon={<LocationIcon className="w-3 h-3 text-emerald-600" />} label={`Branch: ${geoError.branchName ?? ''}`} lat={geoError.branchLat} lng={geoError.branchLng!} color="bg-emerald-50" />
                  )}
