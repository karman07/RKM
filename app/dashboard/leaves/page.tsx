...
const LEAVE_TYPES = [
  { value: 'sick',    label: 'Sick Leave',   iconPath: 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z', activeColor: 'border-red-400 text-red-600',     inactiveText: 'text-slate-400' },
  { value: 'casual',  label: 'Casual Leave', iconPath: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z',                                                                 activeColor: 'border-amber-400 text-amber-600',   inactiveText: 'text-slate-400' },
  { value: 'earned',  label: 'Earned Leave', iconPath: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z', activeColor: 'border-emerald-400 text-emerald-600', inactiveText: 'text-slate-400' },
  { value: 'other',   label: 'Other',        iconPath: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',                             activeColor: 'border-slate-400 text-slate-600',   inactiveText: 'text-slate-400' },
];
...
export default function LeavesPage() {
  ...
  return (
      ...
      <div className="p-6 lg:p-10 max-w-4xl mx-auto space-y-8 bg-white min-h-full">
        {/* Toast */}
        {toast && (
          <div className={`fixed top-6 right-6 z-50 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 text-sm font-bold text-white ${toast.type === 'success' ? 'bg-emerald-600' : 'bg-red-600'}`}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-[#5A0F1A] hover:bg-[#7A1C2A] text-white rounded-2xl text-sm font-bold shadow-lg shadow-[#5A0F1A]/20 transition-all active:scale-95"
          >
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 bg-white transition-all text-center ${
                        form.leave_type === lt.value
                          ? lt.activeColor + ' shadow-sm'
                          : 'border-slate-100 ' + lt.inactiveText + ' hover:border-slate-200'
                      }`}
                    >
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-3.5 border border-slate-200 rounded-2xl text-sm font-bold text-slate-500 hover:bg-slate-50 transition-all">
                  Cancel
                </button>
                <button
                  type="submit"
          ...
          {leaves.length === 0 ? (
            ...
            <div className="divide-y divide-slate-50">
              {leaves.map(leave => {
                ...
                return (
                  <div key={leave._id} className="px-8 py-5 hover:bg-slate-50/50 transition-colors">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-2xl bg-slate-100 flex items-center justify-center flex-shrink-0">
                          <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#64748b" strokeWidth={1.8}>
                            <path strokeLinecap="round" strokeLinejoin="round" d={leaveType?.iconPath || 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'} />
                          </svg>
                        </div>
                        <div>
                      ...
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        ...
                        {leave.admin_note && (
                          <p className="text-[10px] text-slate-400 italic max-w-[200px] text-right">Note: {leave.admin_note}</p>
                        )}
                      </div>
                    </div>
                  </div>
