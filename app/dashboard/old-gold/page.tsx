...
import {
  ...
  FileText, Download, RefreshCw, Sparkles, UploadCloud, ShieldCheck,
} from 'lucide-react';
...
// ── Purity / stone options — sourced entirely from the inventory lookups master (gold purity + stone types), no static lists ──
...
function TxnDetail({
  txn, can, showToast, onChanged,
}: {
  txn: OldGoldTransaction;
  ...
  async function handleGenerate() {
    setGenerating(true);
    try {
      await generateOldGoldForm(txn._id);
      showToast('Buy-back form generated');
      onChanged();
    } catch (e: any) {
  ...
  async function handleUploadSigned(file: File) {
    setUploading(true);
    try {
      await uploadOldGoldSignedForm(txn._id, file);
  ...
  return (
    <tr>
      <td colSpan={8} className="px-6 pb-5 pt-1 bg-slate-50/60">
        ...
        {/* Buy-back form + signed copy */}
        {can('old-gold.manage-form') && (
              ...
              <div className="flex items-center gap-4 px-5 py-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="text-sm font-black text-slate-900">Old Gold Sale Declaration Form</p>
                    {txn.form_url
                  ...
                  <p className="text-[10px] text-slate-400 font-medium truncate">
                    Pre-filled with customer, weight, valuation &amp; T&amp;C — printable, ready to sign.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 pr-4 flex-shrink-0">
                {txn.form_url && (
                  <a href={staticUrl(txn.form_url)} target="_blank" rel="noreferrer"
            ...
            <div className="flex items-stretch gap-0 bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all">
              ...
              <div className="flex items-center gap-4 px-5 py-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-400 font-medium truncate">
                    {txn.signed_form_url
                      ? `Scanned copy on record${txn.signed_form_uploaded_at ? ` · uploaded ${new Date(txn.signed_form_uploaded_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}` : ''}${txn.signed_form_uploaded_by ? ` by ${userName(txn.signed_form_uploaded_by)}` : ''}`
                      : 'Upload the scanned copy once the customer & RSO have signed it.'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 pr-4 flex-shrink-0">
                {txn.signed_form_url && (
                  <a href={staticUrl(txn.signed_form_url)} target="_blank" rel="noreferrer"
                     className="inline-flex items-center gap-1.5 px-3 py-2 border border-slate-200 text-slate-600 text-[11px] font-bold rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all">
                    <Download className="w-3.5 h-3.5" /> View
                  </a>
                )}
