...
function KpiCard({ label, value, sub, icon }: { label: string; value: string; sub?: string; icon: string }) {
  return (
    <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="w-10 h-10 rounded-2xl flex items-center justify-center mb-4 bg-[#2563EB]/10">
          ...
          <div className="w-full relative flex items-end" style={{ height: '120px' }}>
            <div
              ...
              style={{
                ...
                background: d.value > 0 ? '#2563EB' : '#e2e8f0',
              }}
...
export default function MyPersonalSalesPage() {
    ...
    <div className="min-h-screen bg-[#F8F8FA] font-sans">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
          <div>
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#2563EB]">Your Performance</span>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight mt-1">My Sales</h1>
            <p className="text-slate-400 text-sm font-medium mt-0.5">
              Sales recorded under your reference,{' '}
              <span className="text-[#2563EB] font-black">{user?.name ?? 'Manager'}</span>
            </p>
          </div>
          <button
            onClick={() => router.push('/dashboard/sales')}
            className="flex items-center gap-2 text-[11px] font-black text-slate-500 hover:text-[#2563EB] bg-white border border-slate-200 rounded-2xl px-4 py-2 transition-colors"
          >
            View Branch-Wide Overview
            ...
            <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
              {(stats?.recentSales ?? []).map((item: any) => (
                <div key={item._id} className="flex items-center gap-3 p-2.5 rounded-2xl hover:bg-slate-50 transition-colors">
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-[12px] font-black text-[#2563EB]">{fmt(item.selling_price ?? 0)}</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/70 mt-0.5">
                Figures here are scoped to items you personally sold or approved — direct sales carry your manager
                reference, and cashier sale requests you approve keep the cashier's own reference instead. This page
                never shows another staff member's numbers; use the branch-wide overview for that.
              </p>
            </div>
          </div>
        </div>
