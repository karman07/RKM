...
function InventoryContent() {
  ...
  const cartTotal = cart.reduce((sum, it) => sum + (it.live_selling_price ?? it.selling_price), 0);
  const isInCart = (id: string) => cart.some(it => it._id === id);
  function addToCart(item: InventoryItem) {
    setCart(prev => prev.some(it => it._id === item._id) ? prev : [...prev, item]);
  }
  ...
  async function handleScan(barcode: string) {
    ...
    try {
      ...
      if (cartMode) {
        // Cart mode: add the scanned item straight to the bill instead of just previewing it —
        // lets a cashier build a multi-item bill by scanning one item after another.
        if (item.status !== 'available') {
          setScanError(`${item.unique_item_code} is not available for sale (status: ${item.status})`);
        } else if (isInCart(item._id)) {
          showSuccess(`${item.unique_item_code} is already in the bill`);
        } else {
          addToCart(item);
          showSuccess(`Added to bill: ${item.product_id?.name ?? item.unique_item_code}`);
        }
      } else {
        setScannedItem(item);
      }
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-6 bg-white min-h-full">
      ...
      {saleRequestItem && user && (
        <SaleRequestModal
          ...
          onSuccess={() => { setSaleRequestItem(null); showSuccess('Sale request submitted — awaiting manager approval.'); }}
        />
      )}
      {showBatchModal && user && (
        <BatchSaleRequestModal
          items={cart}
          userId={user._id}
          branchId={user.branch?._id ?? ''}
          ...
          onSuccess={(updatedItems) => {
            ...
            setItems(prev => prev.map(it => {
              const updated = updatedItems.find(u => u._id === it._id);
              return updated ? { ...it, ...updated } : it;
            }));
            showSuccess(`Bill for ${updatedItems.length} item${updatedItems.length !== 1 ? 's' : ''} submitted — awaiting manager approval.`);
          }}
        />
      )}
      {successToast && (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-auto">
          <button
            id="cashier-cart-mode-btn"
            onClick={() => setCartMode(v => !v)}
            className={`flex items-center gap-2.5 px-5 py-3 rounded-2xl text-sm font-bold shadow-lg transition-all active:scale-95 ${
              cartMode
                ? 'bg-[#5A0F1A] hover:bg-[#7A1C2A] text-white shadow-[#5A0F1A]/20'
          ...
          >
            ...
            Multi-Item Bill{cartMode && cart.length > 0 ? ` (${cart.length})` : ''}
          </button>
          <button
            id="cashier-scan-btn"
            onClick={() => setShowScanner(true)}
            className="flex items-center gap-2.5 px-5 py-3 bg-[#5A0F1A] hover:bg-[#7A1C2A] text-white rounded-2xl text-sm font-bold shadow-lg shadow-[#5A0F1A]/20 transition-all active:scale-95"
          >
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  {item.status === 'available' && (
                    item.sale_request_status === 'pending' ? (
                      ...
                      isInCart(item._id) ? (
                        <div className="flex-[2] py-2.5 rounded-2xl bg-emerald-50 border border-emerald-200 text-[11px] font-black uppercase tracking-wider text-emerald-700 text-center flex items-center justify-center gap-1.5">
                        ...
                        <button
                          ...
                          className="flex-[2] py-2.5 rounded-2xl border-2 border-[#5A0F1A] text-[#5A0F1A] hover:bg-[#5A0F1A]/5 text-[11px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5"
                        >
      ...
      {cartMode && cart.length > 0 && (
        <div className="fixed bottom-6 right-6 z-[110] w-80 max-w-[92vw]">
          <div className="bg-white border-2 border-[#7A1C2A]/20 rounded-3xl shadow-2xl overflow-hidden">
            <button
              type="button"
              onClick={() => setCartExpanded(v => !v)}
              className="w-full flex items-center justify-between gap-3 px-5 py-4 bg-gradient-to-r from-[#5A0F1A] to-[#7A1C2A] text-white"
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-black text-sm">{cart.length} item{cart.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="flex items-center gap-2.5">
                <span className="font-black text-sm">₹{cartTotal.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</span>
                <svg
                  width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}
                  className={`transition-transform ${cartExpanded ? 'rotate-180' : ''}`}
                >
            ...
            {cartExpanded && (
              <div className="max-h-64 overflow-y-auto divide-y divide-slate-50">
                {cart.map(ci => {
                  const product = typeof ci.product_id === 'object' ? ci.product_id : ({} as any);
                  return (
                    <div key={ci._id} className="flex items-center gap-3 px-4 py-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-black text-slate-900 truncate">{product?.name ?? ci.unique_item_code}</p>
                        <p className="text-[10px] text-slate-400 font-bold truncate">{ci.unique_item_code}</p>
                      </div>
                      <span className="text-xs font-black text-[#5A0F1A] flex-shrink-0">₹{(ci.live_selling_price ?? ci.selling_price).toLocaleString('en-IN')}</span>
                      <button
                        type="button"
            ...
            <div className="p-4">
              <button
                ...
                className="w-full py-3 rounded-2xl bg-[#5A0F1A] hover:bg-[#7A1C2A] text-white font-black text-sm transition-all shadow-lg shadow-[#5A0F1A]/20 flex items-center justify-center gap-2"
              >
