...
export default function ProfilePage() {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-2xl mx-auto space-y-8 bg-white min-h-full">
      ...
      {hasContactDetails && (
        <div className="bg-white border border-slate-100 rounded-[2rem] p-6 sm:p-8 shadow-sm">
          <h2 className="text-base font-black text-slate-900 mb-5">Contact Details</h2>
      ...
      {hasBankDetails && (
        <div className="bg-white border border-slate-100 rounded-[2rem] p-6 sm:p-8 shadow-sm">
          <h2 className="text-base font-black text-slate-900 mb-5">Bank Details</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {user?.account_number && (
              <div className="bg-slate-50 rounded-2xl px-5 py-4">
            ...
            {user?.blank_check_url && (
              <div className="bg-slate-50 rounded-2xl px-5 py-4">
                ...
                <a href={staticUrl(user.blank_check_url)} target="_blank" rel="noreferrer"
                  className="text-sm font-bold text-[#5A0F1A] hover:underline">
                  View Document ↗
                </a>
              </div>
            )}
      ...
      {customFieldEntries.length > 0 && (
        <div className="bg-white border border-slate-100 rounded-[2rem] p-6 sm:p-8 shadow-sm">
          <h2 className="text-base font-black text-slate-900 mb-5">Additional Information</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {customFieldEntries.map(field => {
              const val = user?.custom_field_values?.[field.key];
              return (
                <div key={field._id} className="bg-slate-50 rounded-2xl px-5 py-4">
                  ...
                  {field.type === 'file' ? (
                    <a href={staticUrl(String(val))} target="_blank" rel="noreferrer"
                      className="text-sm font-bold text-[#5A0F1A] hover:underline">View Document ↗</a>
                  ) : field.type === 'url' ? (
                    <a href={String(val)} target="_blank" rel="noreferrer"
                      className="text-sm font-bold text-[#5A0F1A] hover:underline">View ↗</a>
                  ) : (
